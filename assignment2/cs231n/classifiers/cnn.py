...
class ThreeLayerConvNet(object):
    ...
    def __init__(self, input_dim=(3, 32, 32), num_filters=32, filter_size=7,
        ...
        # 2nd layer (hidden affine)
        stride = 2
        H2 = round((H1 - 2)/stride + 1) # 16
        W2 = round((W1 - 2)/stride + 1) # 16
    ...
    def loss(self, X, y=None):
        ...
        # //: floor division =  Quotient when a is divided by b, rounded to the
        # next smallest whole number
        ...
        # 3rd layer : affine backward
        dx, dW3, grads['b3'] = affine_backward(dout, cache3)
        grads['W3'] = dW3 + self.reg * self.params['W3']

        # 2nd layer : affine + Relu backward
        dx, dW2, grads['b2'] = affine_relu_backward(dx, cache2)
        grads['W2'] = dW2 + self.reg * self.params['W2']

        # 1st layer : conv + Relu + Pool backward
